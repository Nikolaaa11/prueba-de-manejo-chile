...
/** Duracion del test real. */
const TEST_SECONDS = 40 * 60;
/** Se aprueba con un maximo de 2 respuestas incorrectas. */
const MAX_ERRORES = 2;

const STORAGE_KEY = "licencia-chile-desafio-v1";
...
/**
 * Saca `RULETA_PICKS` preguntas al azar, asegurando que aparezca al menos una de cada
 * tema critico para que el desafio siempre tenga preguntas de doble puntaje.
 */
function sortearPreguntas(): OficialQuestion[] {
  const pool = barajar(BANCO);
  ...
  for (const q of pool) {
    if (elegidas.length >= RULETA_PICKS) break;
...
export default function DesafioPage() {
  ...
  const persistir = useCallback((next: Guardado) => {
    setDatos(next);
    guardar(next);
  }, []);
  ...
  const [restante, setRestante] = useState(TEST_SECONDS);

  const iniciarRuleta = () => {
    setSorteadas([]);
    setRespuestasTest({});
    setRestante(TEST_SECONDS);
    setFase("ruleta");
  };
  ...
  const alTerminarTest = useCallback(
    (respuestas: Marcadas) => {
      ...
      const aprobado = errores <= MAX_ERRORES;
      persistir({
        ...datos,
    ...
    <div className="space-y-6">
      ...
      {fase === "resultado" && (
        <Resultado
          ...
          tiempoUsado={TEST_SECONDS - restante}
...
function Inicio({
  ...
  const dobles = BANCO.filter((q) => q.critical).length;
      ...
      <div className={`card p-6 ${completo ? "" : "opacity-90"}`}>
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-neutral-500">
              La ruleta gira con las {total} preguntas y saca {RULETA_PICKS}. Tienes{" "}
              <strong>40 minutos</strong> y apruebas con un máximo de{" "}
              <strong>{MAX_ERRORES} respuestas incorrectas</strong>.
            </p>
          </div>
        </div>
        ...
        <ul className="mt-4 grid gap-2 text-sm text-neutral-600 sm:grid-cols-2">
          <li className="flex items-start gap-2">
            <span>⏱️</span> 40 minutos corridos para las {RULETA_PICKS} preguntas.
          </li>
          <li className="flex items-start gap-2">
            <span>❌</span> Máximo {MAX_ERRORES} malas para aprobar.
          </li>
          <li className="flex items-start gap-2">
...
function Estudio({
  ...
  const lista = useMemo(() => {
    if (filtro === "all") return BANCO;
    if (filtro === "pendientes")
      return BANCO.filter((q) => !marcadas[q.n]);
    return BANCO.filter((q) => q.category === filtro);
  ...
  const aciertos = useMemo(
    () =>
      BANCO.filter((x) => marcadas[x.n] && isCorrect(x, marcadas[x.n])).length,
    [marcadas]
  );
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-neutral-500">
          <span className="chip-cyan">
            {respondidas} / {BANCO.length} respondidas
          </span>
          <span className="chip">
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        <Filtro activo={filtro === "pendientes"} onClick={() => setFiltro("pendientes")}>
          Pendientes ({BANCO.length - respondidas})
        </Filtro>
        {(Object.keys(OFICIAL_CATEGORY_LABELS) as OficialCategory[]).map((c) => {
          const n = BANCO.filter((x) => x.category === c).length;
          if (!n) return null;
          return (
  ...
  return (
    <div className="card p-8 text-center">
      <h1 className="text-2xl font-bold">
        🎰 La ruleta de las {BANCO.length} preguntas
      </h1>
      <p className="mt-2 text-neutral-600">
        Gira y saca {RULETA_PICKS} preguntas al azar para tu test real.
      </p>
      ...
      <div className="relative">
        ...
        {etapa === "real" && (
          <div className="animate-fadeUp">
            ...
            <p className="mt-3 text-white/90">
              40 minutos · máximo {MAX_ERRORES} malas · las de ✖️2 valen doble
            </p>
          </div>
        )}
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="mt-2 text-white/90">
          ...
          {preguntas.length} · se permiten hasta {MAX_ERRORES}
        </p>
